...
function vipGod.onSay(player, words, param)
	...
	if action == "check" then
		player:sendTextMessage(MESSAGE_STATUS, string.format('"%s" has %s Premium day(s) left.', targetName, (targetVipDays == 0xFFFF and "infinite" or targetVipDays)))
	elseif action == "adddays" then
		...
		if amount < config.minDays or amount > config.maxDays then
			player:sendTextMessage(MESSAGE_LOOK, string.format("You can only add %d to %d Premium days at a time.", config.minDays, config.maxDays))
			return true
		end
		...
		player:sendTextMessage(MESSAGE_STATUS, string.format('"%s" received %d Premium day(s) and now has %d Premium day(s)', targetName, amount, target:getVipDays()))
	elseif action == "removedays" then
		...
		if amount > targetVipDays then
			...
			player:sendTextMessage(MESSAGE_STATUS, string.format("You removed all Premium days from %s.", targetName))
		else
			target:removePremiumDays(amount)
			player:sendTextMessage(MESSAGE_STATUS, string.format("%s lost %s Premium day(s) and now has %s Premium day(s).", targetName, amount, target:getVipDays()))
		end
